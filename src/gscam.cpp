...
namespace gscam {
    ...
    bool GSCam::configure()
    {
        ...
        ros::param::param<bool>("auto_gain", auto_gain_, false);
        ros::param::param<bool>("auto_exposure", auto_exposure_, false);
        //nh_private_.getParam("auto_gain", auto_gain_);
        //nh_private_.getParam("auto_exposure", auto_exposure_);
        nh_private_.getParam("camera_info_url", camera_info_url_);
        nh_private_.getParam("camera_name", camera_name_);

        // Get the image encoding
        nh_private_.param("image_encoding", image_encoding_, sensor_msgs::image_encodings::BAYER_GBRG8);
    ...
    bool GSCam::init_stream()
    {
        ...
        } else if (image_encoding_ == sensor_msgs::image_encodings::BAYER_GBRG8){
            caps = gst_caps_new_simple( "video/x-raw", 
                    "format", G_TYPE_STRING, "GRB",
                    NULL); 
        }
#else
        if (image_encoding_ == sensor_msgs::image_encodings::RGB8) {
    ...
    void GSCam::publish_stream()
    {
        ...
        if(gst_element_set_state(pipeline_, GST_STATE_NULL) == GST_STATE_CHANGE_FAILURE) {
            ROS_ERROR("Could not reset state of gscam!");
            return;
        }
        ROS_INFO("Reset stream.");
        */
        /*
        ...
        if (!block_until_playing(pipeline_))
        {
            printf("Unable to start pipeline. \n");
        }

        // Set properties when camera is in STATE_PLAYING
        set_bool_or_enum(source, "Exposure Auto", auto_exposure_);
        set_bool_or_enum(source, "Gain Auto", auto_gain_);
        ...
        while(ros::ok())
        {
            ...
            // ROS_INFO_STREAM("Get image width/height.");
            ...
            if (image_encoding_ == "jpeg") {
                ...
                // ROS_INFO_STREAM("Construct image message.");
                ...
                std::copy(
                        ...
                        (buf_data)+(buf_size),
                        img->data.begin());
                // ROS_INFO_STREAM("Copy image data. image encoding: " << image_encoding_);
                // ROS_INFO_STREAM("Publish image message.");
